...
Config.Admin = {
	...
	},
	{
		...
		groups = {'_dev', 'owner', 'superadmin', 'admin'},
		command = function()
			local item = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_NAME', _U('dialogbox_giveitem'), '', 50)
			local amount = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_AMOUNT', _U('dialogbox_giveitemamount'), '', 50)
			if item and amount then
				TriggerServerEvent('giveitem', item, amount)
			end
			RageUI.CloseAll()
		end
	},
	{
		...
		groups = {'_dev', 'owner', 'superadmin', 'admin'},
		command = function()
			local target = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_NAME', _U('dialogbox_setjobid'), '', 50)
			local job = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_AMOUNT', _U('dialogbox_setjob'), '', 50)
			local grade = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_AMOUNT', _U('dialogbox_setjobgrade'), '', 50)
			if target and job then
				TriggerServerEvent('setjob', target, job, grade)
			end
			RageUI.CloseAll()
		end
	},
	{
		name = 'setgroup',
		label = _U('admin_setgroup_button'),
		groups = {'_dev', 'owner', 'superadmin'},
		command = function()
			local target = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_NAME', _U('dialogbox_setjobid'), '', 50)
			local group = KeyboardInput('KORIOZ_BOX_GIVE_ITEM_AMOUNT', _U('dialogbox_setgroup'), '', 50)
			if target and group then
				TriggerServerEvent('setgroup', target, group)
			end
			RageUI.CloseAll()
		end
	},
...
--	},
	...
	},
	{
		...
		groups = {'_dev', 'owner', 'superadmin', 'admin', 'mod'},
		command = function()
			local plyId = KeyboardInput('KORIOZ_BOX_ID', _U('dialogbox_setjobid'), '', 8)

			if plyId then
				TriggerServerEvent('revive', plyId)
			end
