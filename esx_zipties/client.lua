...
AddEventHandler('esx_zipties:notify', function(msg)
	ESX.ShowAdvancedNotification('Kriminel', '', msg , 'CHAR_ARTHUR', 7)
end)
...
AddEventHandler('esx_zipties:setziptie', function()
  ...
  Citizen.CreateThread(function()
    if IsZiptied then
			ESX.ShowAdvancedNotification('Criminal', '', 'Du har fået ~r~strips på', 'CHAR_ARTHUR', 7)
...
AddEventHandler('esx_zipties:unsetziptie', function()
  ...
  Citizen.CreateThread(function()
    if not IsZiptied then
			ESX.ShowAdvancedNotification('Criminal', '', 'Du har fået ~g~strips af', 'CHAR_ARTHUR', 7)
      SetEnableHandcuffs(playerPed, false)
			ClearPedSecondaryTask(playerPed)
      DisablePlayerFiring(playerPed, false)
      SetPedCanPlayGestureAnims(playerPed,  true)
...
AddEventHandler('esx_zipties:autounsetziptie', function()
  ...
  local playerPed = GetPlayerPed(-1)
  Citizen.CreateThread(function()
			ESX.ShowAdvancedNotification('Kriminel', '', 'Stripsne gik i stykker', 'CHAR_ARTHUR', 7)
      SetEnableHandcuffs(playerPed, false)
			ClearPedSecondaryTask(playerPed)
      DisablePlayerFiring(playerPed, false)
      SetPedCanPlayGestureAnims(playerPed,  true)
...
--[[Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
      if IsControlPressed(6, Keys[""]) and IsControlPressed(0 , Keys['LEFTSHIFT']) then
...
function ZiptiePlayer()
	...
	if closestPlayer == -1 or closestDistance > 3.0 then
		ESX.ShowAdvancedNotification('Kriminel', '', 'Ingen spiller i nærheden', 'CHAR_ARTHUR', 7)
	else
		ESX.TriggerServerCallback('esx_zipties:getItemAmount', function(zipties)
			if zipties > 0 then
				if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), "missminuteman_1ig_2", "handsup_enter", 3) then
					...
					ESX.ShowAdvancedNotification('Kriminel', '', 'Spiller ikke ~r~ overgivelse', 'CHAR_ARTHUR', 7)
				end
			else
				ESX.ShowAdvancedNotification('Kriminel', '', 'ikke nok ~r~strips', 'CHAR_ARTHUR', 7)
			end
		end, 'zipties')
	end
end
...
function unZiptiePlayer()
	...
	if closestPlayer == -1 or closestDistance > 3.0 then
		...
		ESX.ShowAdvancedNotification('Kriminel', '', 'Du har ~g~frigivet ~w~dit ~r~victem', 'CHAR_ARTHUR', 7)
	end
end
...
AddEventHandler('esx_zipties:unrestrain', function()
	if IsZiptied then
		...
		IsZiptied = false
		ESX.ShowAdvancedNotification('Kriminel', '', 'Stripsne ~g~ødelagde ~w~du er fri!', 'CHAR_ARTHUR', 7)
		ClearPedSecondaryTask(playerPed)
		SetEnableHandcuffs(playerPed, false)
		DisablePlayerFiring(playerPed, false)
		SetPedCanPlayGestureAnims(playerPed, true)
